...
class GalleryPageController extends PageController {
  ...
  catImageCount(dataBoundContext) {
    return this.imageMap[this.categories[dataBoundContext.index]].length;
  }

  activeCat(dataBoundContext) {
    return this.activeCatIndex == dataBoundContext.index ? "active" : "";
  }

  selectCat(event, dataBoundContext) {
    this.activeCatIndex = dataBoundContext.index;
